...
module FlySDK
  class CreateVolumeRequest
    ...
    attr_accessor :machines_only

    attr_accessor :name

    attr_accessor :region

    attr_accessor :require_unique_zone
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FlySDK::CreateVolumeRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FlySDK::CreateVolumeRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'machines_only')
        self.machines_only = attributes[:'machines_only']
      end
      ...
      if attributes.key?(:'require_unique_zone')
        self.require_unique_zone = attributes[:'require_unique_zone']
      end
