...
module FlySDK
  class ApiTLSOptions
    ...
    attr_accessor :default_self_signed
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FlySDK::ApiTLSOptions` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FlySDK::ApiTLSOptions`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'alpn')
        if (value = attributes[:'alpn']).is_a?(Array)
          self.alpn = value
        end
      end

      if attributes.key?(:'default_self_signed')
        self.default_self_signed = attributes[:'default_self_signed']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          alpn == o.alpn &&
          default_self_signed == o.default_self_signed &&
          versions == o.versions
    end
    ...
    def hash
      [alpn, default_self_signed, versions].hash
    end
