  ...
  class ApiMachineInit
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FlySDK::ApiMachineInit` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FlySDK::ApiMachineInit`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'cmd')
        if (value = attributes[:'cmd']).is_a?(Array)
          self.cmd = value
        end
      end
      ...
      if attributes.key?(:'exec')
        if (value = attributes[:'exec']).is_a?(Array)
          self.exec = value
        end
      end
      ...
      if attributes.key?(:'swap_size_mb')
        self.swap_size_mb = attributes[:'swap_size_mb']
      end
    ...
    def hash
      [cmd, entrypoint, exec, kernel_args, swap_size_mb, tty].hash
    end
